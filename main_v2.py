...
latest_reading = {}
session_data = [] # This list will store all data for the current session

# Use app storage for persistent default values
...
def poll_sensor_and_update_ui():
    """Reads sensor data and updates the live UI elements."""
...
def collect_data_point():
    """Called every 5 minutes to add the latest reading to our session list."""
    if latest_reading:
...
def save_session_to_files():
    """Saves the collected session_data to local and USB paths."""
    ui_timer.deactivate()
...
def start_logging():
    """Clears old data, collects an initial point, and starts the timers."""
    session_data.clear() # Clear data from previous session

    # --- NEW: Manually run the update and collection functions ONCE ---
    # 1. Get the very first sensor reading and update the UI immediately.
    poll_sensor_and_update_ui()
    
    # 2. Add that first reading to our session data list right away.
    #    (The notification inside this function will confirm it was collected)
    collect_data_point()

    # 3. Now, start the timers for all future updates.
    ui_timer.activate()
